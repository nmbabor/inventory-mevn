      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-info">
            ...
            <h4 class="card-title"> Employee Section </h4>
          </div>
...
<script>
import SectionService from '@/services/hr/SectionService'
export default {
  name: 'Units',
  data () {
    return {
  ...
  methods: {
    async getData () {
      const response = await SectionService.index()
      this.allData = response.data
      this.devideHead = Math.ceil(this.allData.length/2)
    },
  async store () {
    await SectionService.store(this.form).then(res=>{
      this.getData()
      this.form.name=''
      this.$swal({
        type: 'success',
    ...
    async updateData (form, i) {
      await SectionService.update(form)
      this.getData()
      this.$swal({
        type: 'success',
