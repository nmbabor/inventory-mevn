      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-info">
            ...
            <h4 class="card-title"> Employee Designation </h4>
          </div>
...
export default {
  ...
  methods: {
    async getData () {
      const response = await DesignationService.index()
      this.allData = response.data
      this.devideHead = Math.ceil(this.allData.length/2)
    },
  async store () {
    await DesignationService.store(this.form).then(res=>{
      this.getData()
      this.form.name=''
      this.$swal({
        type: 'success',
    ...
    async updateData (form, i) {
      await DesignationService.update(form)
      this.getData()
      this.$swal({
        type: 'success',
